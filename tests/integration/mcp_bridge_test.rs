...
fn test_bridge_lists_five_tools() {
    ...
    assert!(info.instructions.as_ref().is_some_and(|i| i.contains("Scavenger")));

    // Access tool_router to list tools
    let tools = bridge.tool_router.list_all();
    let names: Vec<&str> = tools.iter().map(|t| t.name.as_ref()).collect();
    assert_eq!(names.len(), 5, "expected 5 MCP tools, got {}: {:?}", names.len(), names);
...
fn test_bridge_get_capsule_has_file_param() {
    let bridge = make_bridge();
    let tools = bridge.tool_router.list_all();
    let capsule_tool = tools.iter().find(|t| t.name.as_ref() == "get_capsule").unwrap();
    let schema = &capsule_tool.input_schema;
    let props = schema.get("properties").expect("schema should have properties");
    assert!(props.get("file").is_some(), "get_capsule should require 'file' param");
}
...
fn test_bridge_write_annotation_has_text_param() {
    let bridge = make_bridge();
    let tools = bridge.tool_router.list_all();
    let write_tool = tools.iter().find(|t| t.name.as_ref() == "write_annotation").unwrap();
    let schema = &write_tool.input_schema;
    let props = schema.get("properties").expect("schema should have properties");
    assert!(props.get("text").is_some(), "write_annotation should have 'text' param");
...
fn test_bridge_delete_annotation_has_id_param() {
    let bridge = make_bridge();
    let tools = bridge.tool_router.list_all();
    let del_tool = tools.iter().find(|t| t.name.as_ref() == "delete_annotation").unwrap();
    let schema = &del_tool.input_schema;
    let props = schema.get("properties").expect("schema should have properties");
    assert!(props.get("id").is_some(), "delete_annotation should have 'id' param");
}
...
fn test_bridge_search_docs_has_query_param() {
    let bridge = make_bridge();
    let tools = bridge.tool_router.list_all();
    let search_tool = tools.iter().find(|t| t.name.as_ref() == "search_docs").unwrap();
    let schema = &search_tool.input_schema;
    let props = schema.get("properties").expect("schema should have properties");
    assert!(props.get("query").is_some(), "search_docs should have 'query' param");
}
...
fn test_bridge_server_info() {
    ...
    let info = bridge.get_info();
    assert!(
        info.instructions.as_ref().is_some_and(|i| i.contains("capsule")),
